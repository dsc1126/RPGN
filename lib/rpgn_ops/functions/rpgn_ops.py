...
class Voxelization_Idx(Function):
    ...
    def forward(ctx, coords, batchsize, mode=4):
        ...
        RPGN_OP.voxelize_idx(coords, output_coords, input_map, output_map, batchsize, mode)
...
class Voxelization(Function):
    ...
    def forward(ctx, feats, map_rule, mode=4):
        ...
        RPGN_OP.voxelize_fp(feats, output_feats, map_rule, mode, M, maxActive, C)
        return output_feats
    ...
    def backward(ctx, d_output_feats):
        ...
        RPGN_OP.voxelize_bp(d_output_feats.contiguous(), d_feats, map_rule, mode, M, maxActive, C)
...
class PointRecover(Function):
    ...
    def forward(ctx, feats, map_rule, nPoint):
        ...
        RPGN_OP.point_recover_fp(feats, output_feats, map_rule, M, maxActive, C)
    ...
    def backward(ctx, d_output_feats):
        ...
        RPGN_OP.point_recover_bp(d_output_feats.contiguous(), d_feats, map_rule, M, maxActive, C)
...
class BallQueryBatchP(Function):
    ...
    def forward(ctx, coords, batch_idxs, batch_offsets, radius, meanActive):
        ...
        while True:
            ...
            nActive = RPGN_OP.ballquery_batch_p(coords, batch_idxs, batch_offsets, idx, start_len, n, meanActive, radius)
...
class BFSCluster(Function):
    @staticmethod
    def forward(ctx, semantic_label, regional_label, ball_query_idxs, start_len, threshold):
        '''
        :param ctx:
        ...
        assert regional_label.is_contiguous()
        ...
        RPGN_OP.bfs_cluster(semantic_label, regional_label, ball_query_idxs, start_len, cluster_idxs, cluster_offsets, N, threshold)
...
class RoiPool(Function):
    ...
    def forward(ctx, feats, proposals_offset):
        ...
        RPGN_OP.roipool_fp(feats, proposals_offset, output_feats, output_maxidx, nProposal, C)
    ...
    def backward(ctx, d_output_feats):
        ...
        RPGN_OP.roipool_bp(d_feats, proposals_offset, output_maxidx, d_output_feats.contiguous(), nProposal, C)
...
class GetIoU(Function):
    ...
    def forward(ctx, proposals_idx, proposals_offset, instance_labels, instance_pointnum):
        ...
        RPGN_OP.get_iou(proposals_idx, proposals_offset, instance_labels, instance_pointnum, proposals_iou, nInstance, nProposal)
...
class SecMean(Function):
    ...
    def forward(ctx, inp, offsets):
        ...
        RPGN_OP.sec_mean(inp, offsets, out, nProposal, C)
...
class SecMin(Function):
    ...
    def forward(ctx, inp, offsets):
        ...
        RPGN_OP.sec_min(inp, offsets, out, nProposal, C)
...
class SecMax(Function):
    ...
    def forward(ctx, inp, offsets):
        ...
        RPGN_OP.sec_max(inp, offsets, out, nProposal, C)
