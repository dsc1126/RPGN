...
/* ================================== bfs_cluster ================================== */
ConnectedComponent find_cc(Int idx, int *semantic_label, int *regional_label, Int *ball_query_idxs, int *start_len, int *visited){
    ...
    while(!Q.empty()){
        ...
        for(Int i = start; i < start + len; i++){
            ...
            //if(regional_label[idx_i] != 1) continue;
            //printf(cc);
            //int x = label_cur //semantic_label[idx_i]
            //printf(x);
            //cout << "Value of str is : " << cc << endl;
            //sleep(1);
...
//output: clusters, CCs
int get_clusters(int *semantic_label, int *regional_label, Int *ball_query_idxs, int *start_len, const Int nPoint, int threshold, ConnectedComponents &clusters){
    int visited[nPoint] = {0};

    int sumNPoint = 0;

    for(Int i = 0; i < nPoint; i++){
        if((visited[i] == 0) && (regional_label[i] == 1)){
            ConnectedComponent CC = find_cc(i, semantic_label, regional_label, ball_query_idxs, start_len, visited);
    ...
    for(Int i = 0; i < nPoint; i++){
        if((visited[i] == 0) && (regional_label[i] != 1)){ //(visited[i] == 0){
            ConnectedComponent CC = find_cc(i, semantic_label, regional_label, ball_query_idxs, start_len, visited);
            if((int)CC.pt_idxs.size() >= threshold){
                clusters.push_back(CC);
                sumNPoint += (int)CC.pt_idxs.size();
...
void bfs_cluster(at::Tensor semantic_label_tensor, at::Tensor regional_label_tensor, at::Tensor ball_query_idxs_tensor, at::Tensor start_len_tensor,
at::Tensor cluster_idxs_tensor, at::Tensor cluster_offsets_tensor, const int N, int threshold){
    ...
    int *regional_label = regional_label_tensor.data<int>();
    Int *ball_query_idxs = ball_query_idxs_tensor.data<Int>();
    int *start_len = start_len_tensor.data<int>();

    ConnectedComponents CCs;
    int sumNPoint = get_clusters(semantic_label, regional_label, ball_query_idxs, start_len, N, threshold, CCs);
