...
if __name__ == '__main__':
    split = 'val_full_purity_02_03_095_08_diff'
    ...
    for i in range(len(rooms)):
        xyz, rgb, label, instance_label,_ ,_ ,_ ,_ ,_ ,_ = rooms[i]   # label 0~19 -100;  instance_label 0~instance_num-1 -100
        scene_name = files[i].split('/')[-1][:12] #[:13]
        ...
        for inst_id in range(instance_num):
            ...
            if instance_mask.shape[0] != 0: # to avoid empty instance which randomly dropped out during toy data generation
